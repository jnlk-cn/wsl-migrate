...
2. `wsl-migrate` 폴더(`SKILL.md`가 포함된 폴더)를 skill 디렉터리에 복사합니다：
   - **사용자 수준**: `~/.kimi/skills/wsl-migrate/`
   - **프로젝트 수준**: `.agents/skills/wsl-migrate/`
3. Kimi를 재시작하거나 `--skills-dir`로 skills 폴더를 지정하여 실행합니다.
4. WSL 마이그레이션에 관한 질문을 하면 skill이 자동으로 트리거됩니다.

### Claude / Cursor / ChatGPT / Copilot

1. [`SKILL.md`](SKILL.md)를 엽니다.
2. 전체 내용을 에이전트의 컨텍스트(시스템 프롬프트, 사용자 정의 지시사항 또는 채팅 메시지)에 복사합니다.
3. 에이전트에게 WSL 마이그레이션 실행을 요청합니다.

> **팁**: 반복 사용을 위해 `SKILL.md`를 에이전트 설정의 "사용자 정의 지시사항" 또는 "지식 기반"으로 저장하세요.
