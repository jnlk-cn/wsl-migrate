...
2. `wsl-migrate` フォルダ（`SKILL.md` を含むフォルダ）を skill ディレクトリにコピーします：
   - **ユーザーレベル**: `~/.kimi/skills/wsl-migrate/`
   - **プロジェクトレベル**: `.agents/skills/wsl-migrate/`
3. Kimi を再起動するか、`--skills-dir` で skills フォルダを指定して実行します。
4. WSL 移行に関する質問をすれば、skill が自動的にトリガーされます。

### Claude / Cursor / ChatGPT / Copilot

1. [`SKILL.md`](SKILL.md) を開きます。
2. 内容全体をエージェントのコンテキスト（システムプロンプト、カスタム指示、またはチャットメッセージ）にコピーします。
3. エージェントに WSL 移行の実行を依頼します。

> **ヒント**: 繰り返し使用する場合は、`SKILL.md` をエージェント設定の「カスタム指示」または「ナレッジベース」として保存してください。
